export const projects = [
  {
    category: "Featured",
    items: [
      {
        title: "Eta",
        href: "https://eta.js.org",
        description:
          "A super-fast embedded JS template engine that supports Deno. I'm hoping to release a new version soon!",
        image: "eta.png",
      },
      {
        title: "Decline App",
        href: "https://decline.vercel.app/",
        description:
          "A website for practicing Czech, Slovak, and Russian noun declensions.",
        image: "decline.png",
      },
      {
        title: "GOM",
        href: "https://pypi.org/project/gom/",
        description:
          "Pip package with CLI tool to monitor GPU usage across Docker containers. A minimalistic alternative to 'nvidia-smi'. Stands for 'GPU Output Monitor'.",
        image: "gom.png",
      },
    ],
  },
  {
    category: "Other",
    items: [
      {
        title: "npm-to-yarn",
        href: "https://github.com/nebrelbug/npm-to-yarn",
        description:
          "An npm package for converting CLI commands between npm, Yarn, and pnpm.",
        image: "npm-to-yarn.png",
      },
      {
        title: "Squirrelly",
        href: "https://squirrelly.js.org",
        description:
          "A lightweight JavaScript template engine with support for helpers, partials, filters, etc. I'm not actively developing it, but this is the project that helped me get into open source.",
        image: "squirrelly.png",
      },
    ],
  },
  {
    category: "Old",
    items: [
      {
        title: "Splashpad",
        href: "https://chrome.google.com/webstore/detail/splashpad/fainejfmhojphdbbfmpomeknplpdnndb",
        description:
          "A Chrome extension that turns your new tab page into a customizable dashboard.",
        image: "splashpad.png",
      },
      {
        title: "Esperaboard",
        href: "https://chrome.google.com/webstore/detail/esperaboard-esperanto-x-s/nkgbomaneihlabdhjihdhpdlehahahoc",
        description:
          "A Chrome extension to transform characters written in the Esperanto 'x-system' into Esperanto characters while typing.",
        image: "esperaboard.png",
      },
      {
        title: "Tic-Tac-Too",
        href: "https://tictactoe.bengubler.com/",
        description:
          "AI tic-tac-toe bot built with TensorFlow.js. I built this as a teenager, while just starting to learn about ML, so it's definitely not the best code. But it was a great learning experience.",
        image: "tic-tac-too.png",
      },
    ],
  },
];
